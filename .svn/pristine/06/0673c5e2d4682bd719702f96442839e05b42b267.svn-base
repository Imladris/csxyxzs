...
$query_sql = "SELECT * FROM books where upper(book_name) like upper('%".$book_name."%') LIMIT  $offset , $pagesize";
        ...
        if ($pagenum<=10) {
            ...
            for($i=1;$i<=$pagenum;$i++) {
                if($i==$p) {
                    ...
                    echo '<div class="page"><a href="library.php?p='.$i.'&book_name='.$book_name.'" class="page">'.$i.'</a></div>';
                }
            }
        } else {
            for($i=1;$i<=$pagenum;$i++) {
                if($i==$p ) {
                    ...
                    if ( ($i==($p+2) && $i<$pagenum-1)||($i==($p-2) && $i>2) ) {
                        echo '<div class="page"><a href="library.php?p='.$i.'&book_name='.$book_name.'" class="page">'.'...'.'</a></div>';
                    } else {
                        echo '<div class="page"><a href="library.php?p='.$i.'&book_name='.$book_name.'" class="page">'.$i.'</a></div>';
                    }
